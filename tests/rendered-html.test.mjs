...
test("renders the landing page with temporary indexing protection", async () => {
  ...
  assert.match(html, /https:\/\/search\.google\.com\/local\/writereview\?placeid=ChIJQVGWjceLxZQRVEKjluYET9U/);
  assert.doesNotMatch(html, /REVIEW_WRITE_URL_PLACEHOLDER/);
  assert.doesNotMatch(html, /Seleção de avaliações públicas de 5 estrelas/);
